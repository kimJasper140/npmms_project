...
if (isset($_POST['add'])) {
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        "Suggested Retail Price Has been Changed Please be inform<br>:
          product name: <br>".$product_name.
          "Price: ".$price;
        
        $notificationTimestamp = date("Y-m-d H:i:s");
        $notificationSent = 0; // Notification is not sent immediately
    
        $sql_add_notification = "INSERT INTO general_notif_stall ( message, read_status, timestamp) VALUES ( '$notificationMessage',  '$notificationDate', '$notificationSent',notificationTimestamp)";
