...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        $message = "Dear Stall Owner, your stall ownership has been closed.<br> Remarks: $remarks";
        ...
        // Update the stall status to 'closed'
        $sql = "UPDATE stall SET status = '$status' WHERE owner_id = '$stallOwnerId'";
        $conn->query($sql);


        $stallNo = $row['stall_no'];
        $updateStallQuery = "UPDATE available_stall SET status = 'available' WHERE stall_no = '$stallNo'";
        mysqli_query($conn, $updateStallQuery);

        
        // Delete stall owner contracts if the status is 'closed'
        if ($status === 'closed') {
        ...
        echo "<script>alert('Stall owner closedd successfully');</script>";
...
<head>
    ...
    <title>Closed Stall Owner | Admin</title>
    <style>
...
<body>
    <h1>Closed Stall Owner</h1>
    <p>Are you sure yo want to closed the status of the following stall owner?</p>
    ...
    <form method="post">
        ...
        <br>
        <button type="submit">close stall account</button>
    </form>
</body>
